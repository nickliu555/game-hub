...
/**
 * Mount Shoot Ball onto the hub's Express app and HTTP server.
 *
 ...
 */
function mountShootBall(app, httpServer, opts) {
  ...
  setInterval(() => {
    if (Date.now() - lastActivity >= INACTIVITY_RESET_MS) {
      ...
      ns.emit('state:reset');
      broadcastLobby();
      console.log('[shootball] auto-reset after 60 minutes of inactivity.');
      touchActivity();
    }
  }, 60 * 1000).unref();

  // ---------------- Page routes ----------------
  app.get('/shootball/host', (_req, res) => {
    res.sendFile(path.join(__dirname, '..', '..', 'public', 'shootball', 'host.html'));
  });
  app.get('/shootball/join', (_req, res) => {
    res.sendFile(path.join(__dirname, '..', '..', 'public', 'shootball', 'join.html'));
  });
  app.get('/shootball/play', (_req, res) => {
    res.sendFile(path.join(__dirname, '..', '..', 'public', 'shootball', 'player.html'));
  });

  // ---------------- REST endpoints ----------------
  app.get('/api/shootball/config', (_req, res) => {
    const base = getPublicBaseUrl();
    res.json({
      joinUrl: `${base}/shootball/join`,
  ...
  app.get('/api/shootball/qr', async (req, res) => {
