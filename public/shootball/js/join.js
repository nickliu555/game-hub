(function () {
  ...
  const existing = localStorage.getItem('shootball.playerId');
  if (existing) { window.location.replace('/shootball/play'); return; }

  const rejoinName = localStorage.getItem('shootball.rejoinName');
  if (rejoinName) { nameInput.value = rejoinName; localStorage.removeItem('shootball.rejoinName'); }
  ...
  form.addEventListener('submit', function (e) {
    ...
    socket.emit('player:join', { playerId: pid, name: name }, function (res) {
      ...
      localStorage.setItem('shootball.playerId', pid);
      localStorage.setItem('shootball.playerName', res.player.name);
      window.location.replace('/shootball/play');
    });
  });
})();
