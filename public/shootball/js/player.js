(function () {
  ...
  const playerId = localStorage.getItem('shootball.playerId');
  const playerName = localStorage.getItem('shootball.playerName') || 'Player';
  if (!playerId) { window.location.replace('/shootball/join'); return; }
  ...
  socket.on('connect', function () {
    socket.emit('player:reconnect', { playerId: playerId }, function (res) {
      if (!res || !res.ok) {
        localStorage.setItem('shootball.rejoinName', playerName);
        localStorage.removeItem('shootball.playerId');
        window.location.replace('/shootball/join');
        return;
      }
